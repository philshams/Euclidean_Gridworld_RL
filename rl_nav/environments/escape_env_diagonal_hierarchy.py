...
class EscapeEnvDiagonalHierarchy(escape_env.EscapeEnv):
    ...
    def __init__(
        ...
        centroids_ = env_utils.setup_centroids(centroids_path)
        self._centroids = {
            self._inverse_partition_mapping[v]: v for _, v in centroids_.items()
        }

        reachable_partitions = [
            k for k in self._partitions.keys() if k in self._centroids
        ]

        self._super_action_space = list(range(len(self._centroids)))
        self._super_state_space = list(range(len(self._centroids)))
    ...
    def action_space(self) -> List[int]:
        ...
        """
        return self._super_action_space
    ...
    def state_space(self) -> List[Tuple[int]]:
        ...
        Overrides parent property!
        """
        return self._super_state_space
